<!--
Dual License Structure:
Option 1: Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0)
Option 2: Separate written permission for uses outside the public license.
Patent Clause: Patent rights reserved, no patent assertion without written grant.
No pricing/revenue/subscription terms in this document.
-->

Version: 1.0
Priority Date: 2025-10-26
Enhancement Date: 2026-04-30

# Docs

**Explanatory notes, conceptual overviews, and supporting material.**

---

## Contents

- [`symbolic-defense-teaser.md`](./symbolic-defense-teaser.md) — Symbolic defense methodology overview
- [`fractal-dominate-chaos-teaser.md`](./fractal-dominate-chaos-teaser.md) — Fractal chaos management concepts
- [`systems-thinking-tree.md`](./systems-thinking-tree.md) — Systems thinking framework for AI resilience

---

## What This Is

Supporting explanatory material for the Threat Resilience Codex. These documents provide conceptual overviews and introductory material for the research directions documented in the catalog and fundamentals.

---

## Related

- [DNA Codex v5.5](../dna-codex/) — catalog entries
- [Fundamentals](../fundamentals/) — core threat-pattern concepts
- [Whitepapers](../../whitepapers/) — full research papers
---

**Aaron M. Slusher** · [ORCID: 0009-0000-9923-3207](https://orcid.org/0009-0000-9923-3207)
[PUBLICATIONS.md](../../PUBLICATIONS.md) · [aaron@valorgridsolutions.com](mailto:aaron@valorgridsolutions.com)

> Named methodologies and marks referenced in this repository remain the intellectual property of Aaron M. Slusher / ValorGrid Solutions. The license grants use of implementation materials for research and non-commercial purposes; commercial use requires separate permission.

**© 2025–2026 Aaron M. Slusher · ValorGrid Solutions · All Rights Reserved.**
Part of the Synoetic OS™ research ecosystem.
