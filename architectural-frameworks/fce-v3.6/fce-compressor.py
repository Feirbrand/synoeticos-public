...
class FCECompressor:
    ...
    def compress(
        self, content: str, mode: str = "symbolic", target_ratio: float = 8.0
    ) -> Dict:
        """
    ...
    def _estimate_hurst(self, content: str) -> float:
        ...
        variance = sum((l - sum(lengths) / len(lengths)) ** 2 for l in lengths) / len(
            lengths
        )
    ...
    def get_stats(self) -> Dict:
        ...
        stats = {
            "total_compressions": sum(len(v) for v in self.compression_stats.values())
        }
