...
class PhoenixProtocol:
    ...
    def execute_recovery(self, incident_data: Dict) -> RecoverySession:
        ...
        try:
            ...
            if restructuring['recovery_rate'] < self.success_threshold:
                raise Exception(f"Recovery rate {restructuring['recovery_rate']} below threshold")
            ...
            print(f"[Phoenix] Recovery {'SUCCESSFUL' if session.recovery_successful else 'FAILED'}")
    ...
    def get_recovery_stats(self) -> Dict:
        ...
        avg_duration = sum(s.duration_seconds for s in self.recovery_log if s.duration_seconds) / total
...
if __name__ == "__main__":
    ...
    print(f"Duration: {session.duration_seconds:.1f}s ({session.duration_seconds/60:.1f} minutes)")
