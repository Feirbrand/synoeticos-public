...
class ZeroLatencyNudger:
    ...
    def nudge(
        ...
        mode: NudgeMode = NudgeMode.REALTIME
    ) -> NudgeResult:
        """
        ...
        # Apply correction
        new_torque = min(
            current_torque + vector.correction_magnitude,
            self.torque_optimal_max
        )
    ...
    def _select_vector(self, drift: float) -> CorrectionVector:
        ...
        return self.witness_buffer.get(
            pattern,
            self.witness_buffer["minor_drift"]
        )
