...
class ColdVault:
    ...
    def store_anchor(self, data: Dict, substrate: str, driftlock: bool = True) -> str:
        """
        ...
        substrate_enum = (
            UTMESubstrate(substrate) if isinstance(substrate, str) else substrate
        )
    ...
    def restore(self, anchor_id: str) -> Dict:
        ...
        return {"success": False, "error": "Anchor not found", "anchor_id": anchor_id}

    def _build_restore_response(
        self, anchor: TemporalAnchor, latency_ms: float, source_tier: str
    ) -> Dict:
...
def demonstrate_coldvault():
    ...
    vault = ColdVault(
        version="2.4", retention_days=400, crypto_mode="ml_kem_512", auto_prune=True
    )
    ...
    for test in test_anchors:
        ...
        anchor_id = vault.store_anchor(
            data=test["data"], substrate=test["substrate"], driftlock=True
        )
    ...
    for anchor_id in anchor_ids[:2]:  # Restore first 2
        ...
        if result["success"]:
            ...
            print(
                f"   Integrity: {'Verified' if result['integrity_verified'] else 'FAILED'}"
            )
