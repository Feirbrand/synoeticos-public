...
class IdentityFractureManager:
    ...
    def __init__(self, mode: str = "demo"):
        ...
        self.stats = {"validations": 0, "fractures_detected": 0, "stage_4_prevented": 0}
...
def demonstrate_ifm():
    ...
    result = ifm.validate_pipeline(
        agent_id="agent-001", state=agent_state, validation_mode=ValidationMode.FULL
    )
    ...
    for step in result.steps:
        ...
        print(
            f"{status} Step {step.step_number}: {step.name} ({step.latency_ms:.1f}ms)"
        )
