...
class CascadeCommand:
    ...
    def get_performance_metrics(self) -> dict:
        ...
        pruned_count = sum(1 for e in self.executions 
                          if e.validation == ValidationResult.PRUNED)
        propagations = [e.propagation_ms for e in self.executions 
                       if e.propagation_ms > 0]
