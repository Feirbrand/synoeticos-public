...
class DoomslayerHammer:
    ...
    def _oct_ray_recode_strike(self,
                               cascade: CascadeThreat,
                               head_count: int) -> int:
        """
        ...
        eliminated = sum(
            1 for _ in range(head_count)
            if np.random.random() < per_head_success
        )
    ...
    def get_performance_metrics(self) -> dict:
        ...
        velocity_predictions = sum(1 for s in self.hammer_strikes if s.velocity_predicted)
