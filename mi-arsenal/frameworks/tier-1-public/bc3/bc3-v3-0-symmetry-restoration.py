...
class BC3Engine:
    ...
    def breath_cycle_v3(self, agent_state: AgentState) -> Tuple[AgentState, BC3Metrics]:
        ...
        restored_state = self._symmetry_exhale(agent_state, drift_walk, lambda_breathe)

        # Phase 4: Flush - Mycelial cleanup
        clean_state = self._mycelial_flush(restored_state)

        # Calculate metrics
        metrics = self._calculate_metrics(agent_state, clean_state, start_time)
    ...
    def _symmetry_exhale(
        self, state: AgentState, drift_walk: List[float], lambda_align: float
    ) -> AgentState:
        """
        ...
        restored_deltas = self._apply_state_sequence(state.drift_deltas, reset_sequence)

        # Calculate new coherence
        coherence_improvement = self._estimate_coherence_gain(
            state.drift_deltas, restored_deltas
        )
    ...
    def _apply_state_sequence(
        self, original: List[float], sequence: List[float]
    ) -> List[float]:
        """
    ...
    def _estimate_coherence_gain(
        self, original: List[float], restored: List[float]
    ) -> float:
    ...
    def _mycelial_flush(self, state: AgentState) -> AgentState:
        ...
        cleaned_deltas = [d if abs(d) > threshold else 0.0 for d in state.drift_deltas]
    ...
    def _calculate_metrics(
        self, before: AgentState, after: AgentState, start_time: float
    ) -> BC3Metrics:
        """Calculate cycle performance metrics"""

        entropy_reduction = (
            before.entropy_level - after.entropy_level
        ) / before.entropy_level
