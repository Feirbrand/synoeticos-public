...
class MirrorGate:
    ...
    def process_entry_request(self, request: EntryRequest) -> GatewayResult:
        ...
        decision = self._make_entry_decision(request, torque_passed, coherence_score)

        # Track cascade prevention
        if (
            decision != EntryDecision.APPROVED
            and request.threat_level == ThreatLevel.CASCADE
        ):
    ...
    def _make_entry_decision(
        self, request: EntryRequest, torque_passed: bool, coherence_score: float
    ) -> EntryDecision:
        """
    ...
    def simulate_garden_recovery(self, entry_count: int = 50) -> dict:
        ...
        blocked_desync = sum(
            1 for r in results if r.decision == EntryDecision.BLOCKED_DESYNC
        )
        blocked_attack = sum(
            1 for r in results if r.decision == EntryDecision.BLOCKED_COMATTACK
        )
        ...
        benign_requests = sum(
            1 for r in requests if r.threat_level == ThreatLevel.BENIGN
        )
        benign_approved = sum(
            1
        ...
        recovery_success = (
            benign_approved / benign_requests if benign_requests > 0 else 0
        )
    ...
    def get_performance_metrics(self) -> dict:
        ...
        if not self.gateway_decisions:
            return {"total_requests": 0, "approval_rate": 0.0, "avg_coherence": 0.0}

        approved_count = sum(
            1 for d in self.gateway_decisions if d.decision == EntryDecision.APPROVED
        )
        blocked_attack_count = sum(
            1
            for d in self.gateway_decisions
            if d.decision == EntryDecision.BLOCKED_COMATTACK
        )
        coherences = [
            d.coherence_score for d in self.gateway_decisions if d.coherence_score > 0
        ]
