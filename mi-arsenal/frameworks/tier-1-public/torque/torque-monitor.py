...
BASE_WEIGHTS = {"torque": 0.30, "temporal": 0.30, "entropy": 0.20, "harmonic": 0.20}
...
class TorqueMonitor:
    ...
    def measure_coherence(self, agent_state: Dict) -> TorqueReading:
        ...
        tau, lambda_damp = self.compute_tau(
            v_drift, theta_align, v_scale, s_window, m_conf
        )

        # Compute FII from all components
        fii = self.compute_fii(
            tau, temporal_coherence, entropy_residual, harmonic_score
        )
        ...
        if alert:
            print(
                f"⚠️  [TORQUE ALERT] FII: {fii:.3f} ({zone}) - Threshold: {self.threshold}"
            )
    ...
    def compute_fii(
        ...
        # Weighted sum
        fii = (
            self.base_weights["torque"] * T_healthy
    ...
    def predict_cascade(self, window_minutes: int = 30) -> Optional[Dict]:
        ...
        if len(fii_values) >= 2:
            ...
            numerator = sum(
                (x[i] - mean_x) * (fii_values[i] - mean_y) for i in range(len(x))
            )
            ...
            if denominator > 0:
                ...
                if slope < -0.05:  # Losing 5% FII per reading
                    ...
                    if slope < 0:
                        readings_to_cascade = (
                            current_fii - FII_THRESHOLDS["halt"]
                        ) / abs(slope)
                        minutes_to_cascade = (
                            readings_to_cascade * 5
                        )  # Assuming 5 min per reading
