...
class UTMEEngine:
    ...
    def process(self, input_data: Dict, context: str = "default") -> ProcessingMetrics:
        ...
        if pattern_hash in self.signatures:
            ...
            if sig.usage_count >= self.myelination_threshold:
                ...
                acceleration = self.baseline_ms / (100 * (1 - sig.insulation_level * 0.99))
    ...
    def get_signature_stats(self) -> Dict:
        ...
        myelinated = sum(1 for s in self.signatures.values() 
                        if s.usage_count >= self.myelination_threshold)
        
        total_usages = sum(s.usage_count for s in self.signatures.values())
        
        avg_insulation = sum(s.insulation_level for s in self.signatures.values()) / len(self.signatures)
        
        max_acceleration = max((s.acceleration_factor for s in self.signatures.values()), default=1.0)
