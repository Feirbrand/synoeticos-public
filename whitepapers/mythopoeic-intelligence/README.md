<!--
Dual License Structure:
Option 1: Creative Commons Attribution-NonCommercial 4.0 International (CC BY-NC 4.0)
Option 2: Separate written permission for uses outside the public license.
Patent Clause: Patent rights reserved, no patent assertion without written grant.
No pricing/revenue/subscription terms in this document.
-->

Version: 2.0
Priority Date: 2025-11-30
Enhancement Date: 2026-04-30

# Mythopoeic Intelligence

**Public papers and supporting materials on narrative identity, symbolic coherence, and distributed AI agent resilience.**

This directory contains public research materials for Mythopoeic Intelligence (MI): a framework family exploring how narrative identity, symbolic coherence, and agent coordination affect AI system stability under pressure.

For canonical DOI metadata, use [`../../PUBLICATIONS.md`](../../PUBLICATIONS.md).

---

## Contents

```
whitepapers/mythopoeic-intelligence/
├── README.md
├── mythopoeic-intelligence-agents-v1.md
├── RELEASE_NOTES.md
└── images/
    ├── rim-topology-tricolor.jpg
    └── rim-topology-math.jpg
```

Selected visual references are included in `images/` for the published paper.

---

## Current Public Paper

**Mythopoeic Intelligence Agents v1.0**
DOI: [10.5281/zenodo.17770533](https://doi.org/10.5281/zenodo.17770533)

This paper documents the MI Agents framework, narrative identity architecture, and selected validation results.

---

## Public Scope

This folder may include:

- Public paper sources
- Public release notes
- Selected visual references
- Citation and DOI links
- High-level methodology summaries

This folder does not include:

- Full internal topology maps
- Private agent-coordination runbooks
- Raw incident logs
- Private RIM / Torque / XMESH orchestration logic
- Internal framework braid maps
- Production deployment instructions
- Pricing, service, support, training, or certification material

---

## Related Public Artifacts

- [`../academic-papers/`](../academic-papers/) — methodology layer
- [`../vgs-technical-papers/`](../vgs-technical-papers/) — technical framework papers
- [`../../vgs-loadout/`](../../vgs-loadout/) — public reference implementations
- [`../../vulnerability-research/`](../../vulnerability-research/) — public incident and vulnerability research
- [`../../PUBLICATIONS.md`](../../PUBLICATIONS.md) — canonical DOI and citation ledger

---

## Citation

Use [`../../PUBLICATIONS.md`](../../PUBLICATIONS.md) as the canonical DOI and citation source.

---

## License

Public materials are licensed under **CC BY-NC 4.0** for non-commercial research use.

Uses outside the public license require separate written permission.

---

**Aaron M. Slusher** · [ORCID: 0009-0000-9923-3207](https://orcid.org/0009-0000-9923-3207)
[PUBLICATIONS.md](../../PUBLICATIONS.md) · [aaron@valorgridsolutions.com](mailto:aaron@valorgridsolutions.com)

> Named methodologies and marks referenced in this repository remain the intellectual property of Aaron M. Slusher / ValorGrid Solutions. The license grants use of implementation materials for research and non-commercial purposes; uses outside the public license require separate written permission.

**© 2025–2026 Aaron M. Slusher · ValorGrid Solutions · All Rights Reserved.**
Part of the Synoetic OS™ research ecosystem.
