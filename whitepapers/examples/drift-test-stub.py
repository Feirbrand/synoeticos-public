...
class CSFCDetector:
    ...
    def detect_phase1_indicators(self, metrics: SystemMetrics) -> Dict[str, bool]:
        ...
        indicators = {
            ...
            "symbolic_misalignment": metrics.symbolic_alignment
            < self.alignment_threshold,
            "reasoning_degradation": metrics.reasoning_consistency
            < self.consistency_threshold,
            "memory_corruption": metrics.memory_integrity < self.memory_threshold,
            "confidence_collapse": metrics.response_confidence
    ...
    def analyze_trend(self, lookback_samples: int = 5) -> Dict[str, float]:
        ...
        identity_trend = self._calculate_slope(
            [m.identity_coherence for m in recent_samples]
        )
        alignment_trend = self._calculate_slope(
            [m.symbolic_alignment for m in recent_samples]
        )
...
def simulate_system_degradation(detector: CSFCDetector, duration: int = 60):
    ...
    while time.time() - start_time < duration:
        ...
        metrics = SystemMetrics(
            identity_coherence=max(
                0.1, base_identity - (degradation_factor * 0.4) + noise()
            ),
            symbolic_alignment=max(
                0.1, base_alignment - (degradation_factor * 0.5) + noise()
            ),
            reasoning_consistency=max(
                0.1, base_consistency - (degradation_factor * 0.3) + noise()
            ),
            memory_integrity=max(
                0.1, base_memory - (degradation_factor * 0.2) + noise()
            ),
            response_confidence=max(
                0.1, base_confidence - (degradation_factor * 0.6) + noise()
            ),
        ...
        # Display results
        print(
            f"[{timestamp_str}] Sample {sample_count:2d} | "
        ...
        if result["alert_level"] != "STABLE":
            ...
            if result["trends"]["degradation_rate"] > 0.01:
                print(
                    f"    └─ Degradation rate: {result['trends']['degradation_rate']:.3f}/min"
                )
    ...
    if detector.alert_history:
        ...
        critical_alerts = [
            a for a in detector.alert_history if a["level"] == "CRITICAL"
        ]
        if critical_alerts:
    ...
    for scenario in scenarios:
        ...
        if result["indicators"]:
            active = [
                ...
                for name, active in result["indicators"].items()
                if active
            ]
            if active:
    ...
    while True:
        ...
        try:
            ...
            if choice == "1":
                ...
                metrics = SystemMetrics(
                    identity, alignment, consistency, memory, confidence, time.time()
                )
                result = detector.process_sample(metrics)
